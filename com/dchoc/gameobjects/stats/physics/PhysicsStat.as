package com.dchoc.gameobjects.stats.physics
{
   import com.dchoc.gameobjects.stats.Stat;
   import com.dchoc.gameobjects.stats.modifier.StatModifier;
   import com.dchoc.utils.*;
   import nape.phys.Body;
   
   public class PhysicsStat extends Stat
   {
      private var _body:Body;
      
      public function PhysicsStat(param1:String, param2:Body, param3:Number = 0, param4:String = "Group_Base", param5:String = "Normal", param6:Boolean = true)
      {
         super(param1,param3,param4,param5,param6);
         this._body = param2;
      }
      
      override public function clone() : Stat
      {
         var _loc1_:PhysicsStat = super.clone() as PhysicsStat;
         _loc1_._body = this._body;
         return _loc1_;
      }
      
      override public function addModifier(param1:StatModifier) : void
      {
         super.addModifier(param1);
         if(this.body)
         {
            this.updatePhysics();
         }
         else
         {
            LogUtils.log("No body",this,2,"Stats",false,false,false);
         }
      }
      
      override public function removeModifier(param1:StatModifier) : void
      {
         super.removeModifier(param1);
         if(this.body)
         {
            this.updatePhysics();
         }
         else
         {
            LogUtils.log("No body, cannot do updatePhysics",this,2,"Stats",false,false,false);
         }
      }
      
      protected function updatePhysics() : void
      {
         LogUtils.log("Override this in an own class for every property that needs it",this,3,"Stats",true,true,true);
      }
      
      override public function combine(param1:Stat, param2:Boolean = false) : Stat
      {
         var _loc3_:PhysicsStat = super.combine(param1,param2) as PhysicsStat;
         _loc3_._body = this._body;
         return _loc3_;
      }
      
      override public function dispose() : void
      {
         super.dispose();
         this._body = null;
      }
      
      protected function get body() : Body
      {
         return this._body;
      }
   }
}

