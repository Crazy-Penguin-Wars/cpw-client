   ...
   public class Zip extends EventDispatcher
   {
      ...
      public function serialize(stream:IDataOutput, includeAdler32:Boolean = false) : void
      {
         ...
         if(stream != null && this.filesList.length > 0)
         {
            ...
            {
               file = this.filesList[i] as ZipFile;
               if(file != null)
               {
      ...
      {
         return Boolean(this.filesList) ? this.filesList[index] as ZipFile : null;
      }
      
      public function getFileByName(name:String) : ZipFile
      {
         return Boolean(this.filesDict[name]) ? this.filesDict[name] as ZipFile : null;
      }
      
      public function addFile(name:String, content:ByteArray = null, doCompress:Boolean = true) : ZipFile
      {
         return this.addFileAt(Boolean(this.filesList) ? this.filesList.length : 0,name,content,doCompress);
      }
      
      public function addFileFromString(name:String, content:String, charset:String = "utf-8", doCompress:Boolean = true) : ZipFile
      {
         return this.addFileFromStringAt(Boolean(this.filesList) ? this.filesList.length : 0,name,content,charset,doCompress);
      }
      ...
      public function removeFileAt(index:uint) : ZipFile
      {
         ...
         {
            file = this.filesList[index] as ZipFile;
            if(file != null)
            {
