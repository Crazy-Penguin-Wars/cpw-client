package org.as3commons.lang
{
   ...
   public final class ClassUtils
   {
      ...
      public static function forInstance(param1:*, param2:ApplicationDomain = null) : Class
      {
         ...
         if(!(param1 is Proxy) && Boolean(param1.hasOwnProperty(CONSTRUCTOR_FIELD_NAME)))
         {
      ...
      {
         var _loc2_:int = int(param1.indexOf(PACKAGE_CLASS_SEPARATOR));
         if(_loc2_ == -1)
         {
            return param1;
         }
      ...
      public static function getProperties(param1:*, param2:Boolean = false, param3:Boolean = true, param4:Boolean = true, param5:ApplicationDomain = null) : Object
      {
         ...
         for each(node in properties)
         {
            ...
            if(Boolean(node.@uri) && QName(node.@uri).localName != "")
            {
      ...
      public static function getFullyQualifiedImplementedInterfaceNames(param1:Class, param2:Boolean = false, param3:ApplicationDomain = null) : Array
      {
         ...
         var _loc7_:Array = [];
         var _loc8_:XML = getFromObject(param1,param3);
         var _loc9_:XMLList = _loc8_.factory.implementsInterface;
         if(Boolean(_loc9_))
         {
            ...
            while(_loc5_ < _loc4_)
            {
               _loc6_ = _loc9_[_loc5_].@type.toString();
               if(param2)
               {
                  _loc6_ = convertFullyQualifiedName(_loc6_);
               }
      ...
      {
         _describeTypeCache = {};
         if(Boolean(_timer) && Boolean(_timer.running))
         {
            _timer.stop();
         }
      }
      ...
      private static function getFromObject(param1:Object, param2:ApplicationDomain) : XML
      {
         ...
         if(_describeTypeCache.hasOwnProperty(_loc4_))
         {
            _loc3_ = _describeTypeCache[_loc4_];
         }
         else
         {
            if(!_timer)
            ...
            if(!(param1 is Class))
            {
               ...
               {
                  param1 = forName(_loc4_,param2);
               }
            }
            _loc3_ = describeType(param1);
            _describeTypeCache[_loc4_] = _loc3_;
            if(!_timer.running)
            {
               _timer.start();
            }
