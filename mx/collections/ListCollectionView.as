package mx.collections
{
   ...
   public class ListCollectionView extends Proxy implements ICollectionView, IList, IMXMLObject
   {
      ...
      public function getItemIndex(item:Object) : int
      {
         ...
         if(Boolean(this.localIndex) && Boolean(this.sort))
         {
            startIndex = this.mx_internal::findItem(item,Sort.FIRST_INDEX_MODE);
            if(startIndex == -1)
            {
               return -1;
            }
            endIndex = this.mx_internal::findItem(item,Sort.LAST_INDEX_MODE);
      ...
      private function addItemsToView(items:Array, sourceLocation:int, dispatch:Boolean = true) : int
      {
         ...
         var addedItems:Array = Boolean(this.localIndex) ? [] : items;
         ...
         if(Boolean(this.localIndex))
         {
            ...
            for(i = 0; i < items.length; i++)
            {
               ...
               if(this.filterFunction == null || this.filterFunction(item))
               {
                  ...
                  {
                     loc = this.mx_internal::findItem(item,Sort.ANY_INDEX_MODE,true);
                     if(firstOne)
                     {
                        addLocation = loc;
                        firstOne = false;
         ...
         if(Boolean(this.localIndex) && addedItems.length > 1)
         {
            addLocation = -1;
         }
      ...
      private function handlePropertyChangeEvents(events:Array) : void
      {
         ...
         if(Boolean(this.sort) || this.filterFunction != null)
         {
            ...
            for(i = 0; i < events.length; i++)
            {
               ...
               updateEntry.move = updateEntry.move || this.filterFunction || !updateInfo.property || this.sort && this.sort.propertyAffectsSort(String(updateInfo.property));
            }
            eventItems = [];
            for(i = 0; i < updatedItems.length; i++)
            {
               updateEntry = updatedItems[i];
               if(Boolean(updateEntry.move))
               {
                  this.moveItemInView(updateEntry.item,updateEntry.item,eventItems);
               }
               else
               {
      ...
      private function handlePendingUpdates() : void
      {
         ...
         var event:CollectionEvent = null;
         var j:int = 0;
         if(Boolean(this.pendingUpdates))
         {
      ...
      private function internalRefresh(dispatch:Boolean) : Boolean
      {
         ...
         else if(Boolean(this.localIndex))
         {
      ...
      private function removeItemsFromView(items:Array, sourceLocation:int, dispatch:Boolean = true) : void
      {
         ...
         var loc:int = 0;
         var event:CollectionEvent = null;
         var removedItems:Array = Boolean(this.localIndex) ? [] : items;
...
class ListCollectionViewCursor extends EventDispatcher implements IViewCursor
{
   ...
   public function insert(item:Object) : void
   {
      ...
      if(this.afterLast)
      {
         insertIndex = int(this.view.length);
      }
      ...
      {
         insertIndex = int(this.currentIndex);
      }
      ...
      switch(event.kind)
      {
         ...
         case CollectionEventKind.REFRESH:
            if(!(Boolean(this.beforeFirst) || Boolean(this.afterLast)))
            {
               ...
               {
                  if(Boolean(ListCollectionView(view).sort))
                  {
