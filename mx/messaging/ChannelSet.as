package mx.messaging
{
   ...
   public class ChannelSet extends EventDispatcher
   {
      ...
      public function set channels(values:Array) : void
      {
         ...
         var m:int = 0;
         var j:int = 0;
         if(this.mx_internal::configured)
         {
      ...
      protected function setConnected(value:Boolean) : void
      {
         ...
         if(this._connected != value)
         {
            ...
            dispatchEvent(event);
            this.mx_internal::setAuthenticated(value && Boolean(this.currentChannel) && this.currentChannel.authenticated,this._credentials,false);
            if(!this.connected)
            {
      ...
      public function connect(agent:MessageAgent) : void
      {
         if(agent != null && this._messageAgents.indexOf(agent) == -1)
         {
            ...
            if(this.connected && !agent.mx_internal::needsConfig)
            {
      ...
      public function channelConnectHandler(event:ChannelEvent) : void
      {
         ...
         while(this._pendingSends.length > 0)
         {
            ...
            if(command != null)
            {
               ...
               if(!ps.agent.mx_internal::configRequested && ps.agent.mx_internal::needsConfig && command.operation == CommandMessage.CLIENT_PING_OPERATION)
               {
                  command.headers[CommandMessage.NEEDS_CONFIG_HEADER] = true;
                  ps.agent.mx_internal::configRequested = true;
               }
            }
      ...
      public function send(agent:MessageAgent, message:IMessage) : void
      {
         ...
         {
            if(message is CommandMessage && CommandMessage(message).operation == CommandMessage.TRIGGER_CONNECT_OPERATION && !agent.mx_internal::needsConfig)
            {
               ack = new AcknowledgeMessage();
            ...
            if(!this._hasRequestedClusterEndpoints && this.clustered)
            {
               ...
               this._currentChannel.mx_internal::sendInternalMessage(new ClusterMessageResponder(msg,this));
      ...
      mx_internal function authenticationSuccess(agent:AuthenticationAgent, token:AsyncToken, ackMessage:AcknowledgeMessage) : void
      {
         ...
         if(handlingLogin)
         {
            ...
            this.currentChannel.mx_internal::setAuthenticated(true);
         }
         else
         {
            ...
            this.currentChannel.mx_internal::setAuthenticated(false);
         }
      ...
      protected function sendHeartbeat() : void
      {
         ...
         if(pollingChannel != null && pollingChannel.mx_internal::_shouldPoll)
         {
            return;
         }
         ...
         this.currentChannel.mx_internal::sendInternalMessage(new MessageResponder(null,heartbeat));
      }
...
class ClusterMessageResponder extends MessageResponder
{
   ...
   override protected function resultHandler(message:IMessage) : void
   {
      ...
      if(message.body != null && (message.body is Array || message.body is ArrayCollection))
      {
         ...
         this._channelSet.mx_internal::channelFailoverURIs = channelFailoverURIs;
      }
   }
}
