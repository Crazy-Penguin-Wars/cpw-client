   ...
   public class Channel extends EventDispatcher implements IMXMLObject
   {
      ...
      protected function setConnected(value:Boolean) : void
      {
         ...
         if(this._connected != value)
         {
            ...
            if(!value)
            {
               this.mx_internal::setAuthenticated(false);
            }
         }
      }
      ...
      mx_internal function setAuthenticated(value:Boolean) : void
      {
         ...
         if(value != this._authenticated)
         {
            ...
            for(i = 0; i < this._channelSets.length; i++)
            {
               ...
               cs.mx_internal::setAuthenticated(this.authenticated,this.credentials);
            }
            dispatchEvent(event);
         }
      }
      ...
      protected function applyClientLoadBalancingSettings(props:XML) : void
      {
         ...
         var urlCount:int = int(clientLoadBalancingProps.url.length());
         if(urlCount == 0)
         {
            return;
         }
      ...
      final public function connect(channelSet:ChannelSet) : void
      {
         ...
         else if(!this._connecting)
         {
            ...
            if(FlexClient.getInstance().id == null)
            {
               ...
               if(!flexClient.mx_internal::waitForFlexClientId)
               {
                  flexClient.mx_internal::waitForFlexClientId = true;
      ...
      public function logout(agent:MessageAgent) : void
      {
         ...
         if(this.connected && this.authenticated && this.credentials || this.mx_internal::authenticating && this.credentials)
         {
            ...
            this.mx_internal::authenticating = true;
         }
      ...
      public function setCredentials(credentials:String, agent:MessageAgent = null, charset:String = null) : void
      {
         ...
         if(this.mx_internal::authenticating && changedCreds)
         {
      ...
      protected function connectSuccess() : void
      {
         ...
         var j:int = 0;
         this.shutdownConnectTimer();
         if(ServerConfig.mx_internal::fetchedConfig(this.endpoint))
         {
      ...
      protected function flexClientWaitHandler(event:PropertyChangeEvent) : void
      {
         ...
         if(event.property == "waitForFlexClientId")
         {
            ...
            if(flexClient.mx_internal::waitForFlexClientId == false)
            {
      ...
      protected function setFlexClientIdOnMessage(message:IMessage) : void
      {
         ...
         message.headers[AbstractMessage.FLEX_CLIENT_ID_HEADER] = id != null ? id : FlexClient.mx_internal::NULL_FLEXCLIENT_ID;
      }
      ...
      {
         return this._shouldBeConnected && (this._previouslyConnected || this.mx_internal::reliableReconnectDuration != -1 || this._failoverURIs != null && this._failoverURIs.length > 0);
      }
         ...
         if(this._previouslyConnected)
         {
            ...
            if(acs != null)
            {
               for each(channelSet in this.channelSets)
               {
                  ...
                  {
                     d = int((channelSet as acs)["reliableReconnectDuration"]);
                     if(d > duration)
                     {
                        duration = d;
                     }
            ...
            if(duration != -1)
            {
               ...
               this.mx_internal::reliableReconnectDuration = duration;
               this._reliableReconnectBeginTimestamp = new Date().valueOf();
               new AsyncDispatcher(this.reconnect,null,1);
               return;
            }
         }
         if(this.mx_internal::reliableReconnectDuration != -1)
         {
            this._reliableReconnectLastTimestamp = new Date().valueOf();
            remaining = this.mx_internal::reliableReconnectDuration - (this._reliableReconnectLastTimestamp - this._reliableReconnectBeginTimestamp);
            if(remaining > 0)
            {
               delay = 1000;
      ...
      {
         this.mx_internal::reliableReconnectDuration = -1;
