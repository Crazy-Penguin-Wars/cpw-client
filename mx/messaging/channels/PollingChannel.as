package mx.messaging.channels
{
   ...
   public class PollingChannel extends Channel
   {
      ...
      protected function set internalPollingEnabled(value:Boolean) : void
      {
         ...
         if(!value && (this.mx_internal::timerRunning || !this.mx_internal::timerRunning && this.mx_internal::_pollingInterval == 0))
         {
            this.mx_internal::stopPolling();
         }
         else if(value && this.mx_internal::_shouldPoll && !this.mx_internal::timerRunning)
         {
            this.startPolling();
         }
      }
      ...
      mx_internal function set internalPollingInterval(value:Number) : void
      {
         ...
         if(value == 0)
         {
            this.mx_internal::_pollingInterval = value;
         ...
         else
         {
            ...
            if(this.mx_internal::_timer != null)
            {
               this.mx_internal::_timer.delay = this.mx_internal::_pollingInterval = value;
      ...
      protected function applyPollingSettings(settings:XML) : void
      {
         ...
         {
            this.mx_internal::internalPollingInterval = parseInt(props[POLLING_INTERVAL_MILLIS].toString());
         }
         else if(props[POLLING_INTERVAL_LEGACY].length() != 0)
         {
            this.mx_internal::internalPollingInterval = parseInt(props[POLLING_INTERVAL_LEGACY].toString()) * 1000;
         }
...
class PollCommandMessageResponder extends MessageResponder
{
   ...
   override protected function statusHandler(msg:IMessage) : void
   {
      ...
      pollingChannel.mx_internal::stopPolling();
      ...
      {
         pollingChannel.mx_internal::pollFailed(true);
      }
      else
      {
         pollingChannel.mx_internal::pollFailed(false);
      }
   }
   ...
   {
      var pollingChannel:PollingChannel = PollingChannel(channel);
      if(pollingChannel.connected && pollingChannel.mx_internal::_shouldPoll)
      {
         if(adaptivePollWait == 0)
         {
            if(pollingChannel.mx_internal::internalPollingInterval == 0)
            {
               pollingChannel.poll();
            }
            else if(!pollingChannel.mx_internal::timerRunning)
            {
               pollingChannel.mx_internal::_timer.delay = pollingChannel.mx_internal::_pollingInterval;
               pollingChannel.mx_internal::_timer.start();
            }
         }
         else
         {
            pollingChannel.mx_internal::_timer.delay = adaptivePollWait;
            pollingChannel.mx_internal::_timer.start();
         }
      }
   }
