package mx.messaging
{
   ...
   public class AbstractConsumer extends MessageAgent
   {
            ...
            switch(op)
            {
               ...
               case CommandMessage.POLL_OPERATION:
                  if(ackMsg.body != null && ackMsg.body is Array)
                  {
                     ...
                     {
                        this.mx_internal::messageHandler(MessageEvent.createEvent(MessageEvent.MESSAGE,message));
                     }
                  }
                  super.acknowledge(ackMsg,msg);
            }
      ...
      override public function channelConnectHandler(event:ChannelEvent) : void
      {
         ...
         if(connected && channelSet != null && channelSet.currentChannel != null && !channelSet.currentChannel.mx_internal::realtime && Log.isWarn())
         {
      ...
      public function unsubscribe(preserveDurable:Boolean = false) : void
      {
         ...
         if(this.subscribed)
         {
            if(channelSet != null)
            {
               channelSet.removeEventListener(destination,this.mx_internal::messageHandler);
            }
            if(Log.isInfo())
            {
