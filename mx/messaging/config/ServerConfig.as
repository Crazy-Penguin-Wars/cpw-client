package mx.messaging.config
{
   ...
   public class ServerConfig
   {
      ...
      {
         var channelIdsA:Array = mx_internal::getChannelIdList(destinationA);
         var channelIdsB:Array = mx_internal::getChannelIdList(destinationB);
      ...
      mx_internal static function channelSetMatchesDestinationConfig(channelSet:ChannelSet, destination:String) : Boolean
      {
         ...
         if(channelSet != null)
         {
            if(ObjectUtil.compare(channelSet.channelIds,mx_internal::getChannelIdList(destination)) == 0)
            {
               return true;
            }
            csUris = [];
            ...
            ids = mx_internal::getChannelIdList(destination);
            dsUris = [];
      ...
      mx_internal static function getChannelIdList(destination:String) : Array
      {
         ...
         return Boolean(destinationConfig) ? getChannelIds(destinationConfig) : getDefaultChannelIds();
      }
      ...
      private static function internalGetChannelSet(destinationConfig:XML, destinationId:String) : ChannelSet
      {
         ...
         heartbeatMillis = int(serverConfigData["flex-client"]["heartbeat-interval-millis"]);
