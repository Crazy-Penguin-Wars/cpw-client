package mx.styles
{
   ...
   public class CSSMergedStyleDeclaration extends CSSStyleDeclaration
   {
      ...
      public function CSSMergedStyleDeclaration(style:CSSStyleDeclaration, parentStyle:CSSStyleDeclaration, selector:Object = null, styleManager:IStyleManager2 = null, setSelector:Boolean = false)
      {
         ...
         if(Boolean(style) && Boolean(style.mx_internal::effects))
         {
            mx_internal::effects = [];
            effectsArray = style.mx_internal::effects;
         ...
         if(Boolean(parentStyle) && Boolean(parentStyle.mx_internal::effects))
         {
            if(!mx_internal::effects)
            {
               mx_internal::effects = [];
            }
            effectsArray = parentStyle.mx_internal::effects;
            n = effectsArray.length;
            for(i = 0; i < n; i++)
            {
               mx_internal::effects[i] = effectsArray[i];
      ...
      {
         if(Boolean(this.style))
         {
            return this.style.mx_internal::addStyleToProtoChain(chain,target,filterMap);
         }
         if(Boolean(this.parentStyle))
         {
            return this.parentStyle.mx_internal::addStyleToProtoChain(chain,target,filterMap);
         }
         return chain;
      }
   }
