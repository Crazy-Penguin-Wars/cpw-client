package mx.rpc.http
{
   ...
   public class AbstractOperation extends mx.rpc.AbstractOperation
   {
      ...
      public function set resultFormat(value:String) : void
      {
         ...
         switch(value)
         {
            ...
            case mx_internal::RESULT_FORMAT_FLASHVARS:
               break;
            default:
               if(value != null && (sf = SerializationFilter.mx_internal::filterForResultFormatTable[value]) == null)
               {
                  message = this.mx_internal::resourceManager.getString("rpc","invalidResultFormat",[value,mx_internal::RESULT_FORMAT_OBJECT,mx_internal::RESULT_FORMAT_ARRAY,mx_internal::RESULT_FORMAT_XML,mx_internal::RESULT_FORMAT_E4X,mx_internal::RESULT_FORMAT_TEXT,mx_internal::RESULT_FORMAT_FLASHVARS]);
      ...
      {
         if(this.mx_internal::_rootURL == null)
         {
            this.mx_internal::_rootURL = LoaderConfig.url;
         }
         return this.mx_internal::_rootURL;
      }
      
      public function set rootURL(value:String) : void
      {
         this.mx_internal::_rootURL = value;
      }
      ...
      public function sendBody(parameters:Object) : AsyncToken
      {
         ...
         if(Concurrency.SINGLE == this.concurrency && Boolean(mx_internal::activeCalls.hasActiveCalls()))
         {
            token = new AsyncToken(null);
            m = this.mx_internal::resourceManager.getString("rpc","pendingCallExists");
         ...
         if(ctype == mx_internal::CONTENT_TYPE_XML)
         {
            ...
            else if(!(parameters is XMLNode) && !(parameters is XML))
            {
               if(this.xmlEncode != null)
               {
                  ...
                  {
                     token = new AsyncToken(null);
                     msg = this.mx_internal::resourceManager.getString("rpc","xmlEncodeReturnNull");
                  ...
                  {
                     token = new AsyncToken(null);
                     msg = this.mx_internal::resourceManager.getString("rpc","xmlEncodeReturnNoXMLNode");
                     fault = new Fault(ERROR_ENCODING,msg);
                     faultEvent = FaultEvent.createEvent(fault,token);
                     new AsyncDispatcher(mx_internal::dispatchRpcEvent,[faultEvent],10);
                     return token;
         ...
         else
         {
            ...
            {
               token = new AsyncToken(null);
               msg = this.mx_internal::resourceManager.getString("rpc","urlNotSpecified");
         ...
         return this.mx_internal::invoke(message);
      }
      ...
      override mx_internal function processResult(message:IMessage, token:AsyncToken) : Boolean
      {
         ...
         this.mx_internal::_log.info("Decoding HTTPService response");
         this.mx_internal::_log.debug("Processing HTTPService response message:\n{0}",message);
         ...
         if(body is String)
         {
            if(this.resultFormat == mx_internal::RESULT_FORMAT_XML || this.resultFormat == mx_internal::RESULT_FORMAT_OBJECT || this.resultFormat == mx_internal::RESULT_FORMAT_ARRAY)
            {
               ...
               if(this.resultFormat == mx_internal::RESULT_FORMAT_OBJECT || this.resultFormat == mx_internal::RESULT_FORMAT_ARRAY)
               {
                  if(this.xmlDecode != null)
                  {
                     ...
                     if(decoded == null)
                     {
                        msg = this.mx_internal::resourceManager.getString("rpc","xmlDecodeReturnNull");
                  ...
                  else
                  {
                     ...
                     if(decoded == null)
                     {
                        msg = this.mx_internal::resourceManager.getString("rpc","defaultDecoderFailed");
            ...
            {
               mx_internal::_result = this.decodeParameterString(String(body));
            }
            else
            {
               mx_internal::_result = body;
      ...
      override mx_internal function preHandle(event:MessageEvent) : AsyncToken
      {
         ...
         var wasLastCall:Boolean = Boolean(mx_internal::activeCalls.wasLastCall(AsyncMessage(event.message).correlationId));
