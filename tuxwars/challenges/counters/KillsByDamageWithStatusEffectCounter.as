package tuxwars.challenges.counters
{
   ...
   {
      public function KillsByDamageWithStatusEffectCounter(param1:Challenge, param2:String, param3:int, param4:String, param5:ChallengeParamReference)
      {
      ...
      override public function handlePlayerDied(param1:ChallengePlayerDiedMessage) : void
      {
         ...
         if(playerId != _loc6_._id)
         {
            if(_loc5_)
            {
               if(Boolean(_loc5_.gameObject) && _loc9_._id == playerId)
               {
                  if(params)
                  {
                     ...
                     if(_loc3_ != null)
                     {
                        for each(_loc7_ in _loc3_)
                        {
                           ...
                           {
                              for each(_loc9_ in _loc4_.followerIdsAtDeath)
                              {
                                 if(params.statusIDs.indexOf(_loc9_) != -1)
                                 {
                        ...
                        _loc8_ = _loc4_;
                        LogUtils.log(toString() + " Player: " + _loc8_._id + " params: " + params.toString() + " compared to: (" + _loc4_.followerIdsAtDeath.toString() + ") and (" + _loc3_.toString() + ")",this,0,"Challenges",false,false,false);
                     }
                     else
                     {
